    ...
    module NestedSet #:nodoc:
      module ActiveRecordModel
        class Tree
          ...
          delegate(
            :left_column_name, :right_column_name, :quoted_parent_column_full_name,
          ...
          def rebuild!
            ...
            root_nodes.each do |root_node|
              ...
              reset_left_and_rights(root_node)
          ...
          def reset_left_and_rights(node)
            set_left!(node)
            # find
            node_children(node).each { |n| reset_left_and_rights(n) }
            set_right!(node)
