      ...
      module MongoidModel
        module Relatable
          ...
          def self_and_ancestors
            nested_set_scope
              .lte(left_column_name => left)
              .gte(right_column_name => right)
          end
          ...
          def leaves
            descendants.where(
              "this[#{quoted_right_column_name}] - this[#{quoted_left_column_name}] == 1"
            )
          end
          ...
          def self_and_descendants
            # using _left_ for both sides here lets us benefit
            # from an index on that column if one exists
