...
describe 'AwesomeNestedSet' do
    ...
    context 'when items have children' do
      it 'updates their counter_cache' do
        expect { subject }.to change {
                                things(:parent1).reload.children_count
                              } .to(2).from(0)
  ...
  it 'multi_scoped_rebuild' do
    ...
    expect(Note.roots.find_by(body: 'A')).to eq(root)
    expect([child1, child2]).to eq(Note.roots.find_by(body: 'A').children)
  end
  ...
  it 'quoting_of_multi_scope_column_names' do
    ...
    expected_quoted_scope_column_names = if Note.connection.adapter_name =~ /oracle/i
                                           ['"NOTABLE_ID"', '"NOTABLE_TYPE"']
                                         elsif Note.connection.adapter_name =~ /mysql/i
                                           ['`notable_id`', '`notable_type`']
                                         else
                                           ['"notable_id"', '"notable_type"']
                                         end
  ...
  describe 'option dependent' do
    ...
    describe 'model with default_scope' do
      it 'should have correct #lft & #rgt' do
        ...
        DefaultScopedModel.send(:default_scope, proc { parent.reload.self_and_descendants })
